...
public class MavenComponentType extends AbstractComponentType
{
    ...
    static void addDependencies(IMavenProject project, IComponentReader reader, CSpecBuilder cspec, GroupBuilder archives)
    throws CoreException
    {
        ...
        {
            addDependency( query, provider, cspec, archives, dependency.getGroupId(), dependency.getArtifactId(), dependency.getVersion() );
        }
    }

    static List<Dependency> getDependencies( IMavenProject project )
    {
        List<Dependency> projectDependencies = project.getMavenProject().getDependencies();
        List<Dependency> dependencies = new ArrayList<Dependency>( projectDependencies.size() );
    ...
    static IVersionDesignator createVersionDesignator(String versionStr) throws CoreException
    {
        ...
        char startChar = versionStr.charAt( 0 );
        IVersionDesignator vd;

        /* if it is a range leave it as is */
        if ((startChar == '[') || (startChar == '('))
        {
            vd = VersionFactory.createDesignator(VersionFactory.TripletType, versionStr);
        }
        /* else Buckminster needs to bracket versions to mean an exact match
         * but as it doesn't have conflict resolution it will fail when two different versions are used */
        else
        {
            // TODO use exact version when possible, when there's a conflict resolution mechanism
            // vd = createExactVersionDesignator(versionStr);
            vd = createGTEqualVersionDesignator(versionStr);
        }
        
        return vd;
    }

    private static IVersionDesignator createExactVersionDesignator(String versionStr) throws CoreException
    {
        String v = "["+versionStr+","+versionStr+"]";
        return VersionFactory.createDesignator(VersionFactory.TripletType, v);
    }

    private static IVersionDesignator createGTEqualVersionDesignator(String versionStr) throws CoreException
    {
    ...
    /**
     * Create a {@link VersionMatch} from the string representation from Maven pom
     * 
     * @param versionStr
     * @param typeInfo
     * @return the version match
     * @throws CoreException
     */
    static VersionMatch createVersionMatch(String versionStr, String typeInfo) throws CoreException
	{
		...
		if(m.matches())
		{
			/* we don't use latest() on purpose */
            //versionStr = m.group(1);
		    //return new VersionMatch(createVersion(versionStr), VersionSelectorFactory.latest(versionStr, typeInfo));
			return new VersionMatch(createVersion(versionStr), VersionSelectorFactory.tag(versionStr, typeInfo));
		}
			...
			GroupBuilder archives, ExpandingProperties properties, Node dep) throws CoreException
	{
        ...
        addDependency( query, provider, cspec, archives, groupId, artifactId, versionStr );
	}
	
    private static void addDependency(ComponentQuery query, Provider provider, CSpecBuilder cspec, GroupBuilder archives, String groupId, String artifactId, String version)
    throws CoreException
    {
		...
		if(vd == null)
			vd = createVersionDesignator(version);
