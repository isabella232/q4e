...
public class EclipseMaven implements IMaven {
    ...
    public IMavenProject getMavenProject(IFile projectSpecification, boolean resolveTransitively) throws CoreException {
    ...
    private IMavenProject getMavenProject(EclipseMavenProject mavenProject, boolean resolveTransitively)
        ...
        try {
            if (resolveTransitively) {
                MavenExecutionResult status = mavenEmbedder.readProjectWithDependencies(generateRequest(mavenProject,
                        null));
    ...
    public IMavenProject getMavenProject(Artifact artifact, List<ArtifactRepository> remoteArtifactRepositories)
        ...
        try {
            if (mavenProjectBuilder == null) {
                mavenProjectBuilder = (MavenProjectBuilder) mavenEmbedder.getPlexusContainer().lookup(
                        MavenProjectBuilder.ROLE);
            }
            MavenProject mavenRawProject = mavenProjectBuilder.buildFromRepository(artifact,
                    remoteArtifactRepositories, mavenEmbedder.getLocalRepository(), true);
    ...
    public List<ArtifactVersion> getArtifactVersions(Artifact artifact, List<ArtifactRepository> remoteRepositories)
        ...
        if (artifactMetadataSource == null) {
            try {
                artifactMetadataSource = (ArtifactMetadataSource) mavenEmbedder.getPlexusContainer().lookup(
